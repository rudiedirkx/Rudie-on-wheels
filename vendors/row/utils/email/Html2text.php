...
namespace row\utils\email;

class Html2text {


	static public function quick( $html ) {
		$h2t = new self($html);
		return $h2t->get_text();
	}
	...
	public function _convert()
	{
		...
		$text = trim(stripslashes(str_replace("\r", '', $this->html)));
	...
	public function _build_link_list( $link, $display )
	{
		if ( substr($link, 0, 7) == 'http://' || substr($link, 0, 8) == 'https://' || substr($link, 0, 7) == 'mailto:' ) {
			$this->_link_count++;
			$this->_link_list .= "[" . $this->_link_count . "] ".str_replace('mailto:', '', $link)."\n";
			$additional = is_int(strpos($link, $display)) ? '' : ' (' . str_replace('mailto:', '', $link) . ')';
